...
import { steps } from "@features/home/utility/data";
...
export default function Workflow() {
  return (
    <section id="workflow" className="w-full overflow-hidden bg-[#f6f3ed]">
      <div className="grid min-h-180 lg:grid-cols-[0.88fr_1.12fr]">
        <Reveal className="flex flex-col justify-center px-7 py-16 sm:px-12 lg:px-16">
          ...
          >
            FreshFlow turns receiving, serving, and dispatch into one live route
            so every team member knows what needs to move next.
          </motion.p>
        ...
        <Reveal className="relative overflow-hidden bg-[#153a20] px-7 py-16 text-white sm:px-12 lg:px-16">
          ...
          <motion.svg
            variants={fade}
            viewBox="0 0 900 520"
            className="absolute inset-x-0 top-24 z-0 mx-auto h-105 w-[92%] text-[#ffe078]"
          >
            <motion.path
          ...
          <motion.div
            ...
            className="absolute left-[8%] top-[25%] z-20 grid size-16 place-items-center rounded-full bg-[#ffe078] text-black shadow-2xl shadow-yellow-900/20"
          >
            <Navigation className="size-7" />
          </motion.div>

          <div className="relative z-10 grid min-h-140 gap-5 lg:block">
            {steps.map(([title, copy, Icon], index) => (
              ...
              >
                <div className="mb-5 flex items-center justify-between">
                  <span className="grid size-14 place-items-center rounded-full bg-[#f6f3ed] text-[#f1950c]">
            ...
            <motion.div
              variants={fade}
              className="relative rounded-full bg-white/10 p-3 backdrop-blur-md lg:absolute lg:bottom-5 lg:left-6 lg:right-6"
            >
              <div className="flex items-center justify-between rounded-full bg-white px-5 py-4 text-black">
                <span className="flex items-center gap-2 font-black">
                  <MapPin className="size-5 text-[#f1950c]" /> Dispatch board
                </span>
                <span className="rounded-full bg-[#d9ffb9] px-4 py-2 text-sm font-black">
                  On time
                </span>
              </div>
            </motion.div>
