...
} from "@features/deliveries/schema/deliveryConfig";
import { fade, panel, fieldInput as inputClass } from "@shared/theme";
...
function formFromDelivery(delivery) {
  return {
    ...
    scheduledAt: delivery.scheduledAt
      ? String(delivery.scheduledAt).slice(0, 16)
      : "",
  };
}
...
function Hero({ metrics }) {
  return (
    <motion.section
      variants={fade}
      className={`${panel} relative isolate overflow-hidden bg-[#153a20] p-6 text-white sm:p-8`}
    >
      ...
      <motion.div
        ...
        className="absolute right-8 top-10 hidden size-24 rounded-full bg-[#ffe078]/25 md:block"
      />
      ...
      <h1 className="mt-8 max-w-3xl text-4xl font-black tracking-tight sm:text-5xl">
        Deliveries with every handoff visible.
      </h1>
      <p className="mt-4 max-w-2xl text-base font-medium leading-7 text-white/70">
        Assign orders, track riders, and move delivery jobs from pending to
        delivered without losing context.
      </p>
...
function DeliveryForm({
  ...
  return (
    <motion.section variants={fade} className={`${panel} p-5 sm:p-6`}>
      ...
      {(error || message) && (
        <div
          className={`mt-5 rounded-2xl px-4 py-3 text-sm font-bold ${error ? "bg-red-50 text-red-600" : "bg-[#d9ffb9] text-[#153a20]"}`}
        >
          {error || message}
        </div>
      )}
      <form onSubmit={onSubmit} className="mt-5 space-y-5">
        ...
        <div className="flex gap-3">
          <button
            disabled={saving}
            className="inline-flex flex-1 items-center justify-center gap-2 rounded-full bg-[#153a20] px-5 py-3 text-sm font-black text-white disabled:opacity-60"
          >
          ...
          {editingId && (
            <button
              type="button"
              onClick={onCancel}
              className="rounded-full bg-[#f8faf7] px-5 py-3 text-sm font-black text-zinc-600 hover:bg-[#d9ffb9]"
            >
              Cancel
            </button>
          )}
...
function DeliveryCard({ delivery, deletingId, onEdit, onRemove, onStatus }) {
  return (
    <motion.article
      layout
      whileHover={{ y: -4 }}
      className="rounded-3xl shadow-xl border border-gray-300 bg-[#f8faf7] p-4"
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="mt-2 text-sm font-bold text-zinc-500">
            {delivery.customerName} / {delivery.address || "No address"}
          </p>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {nextDeliveryStatuses(delivery.status).map((item) => (
            <button
              key={item}
              type="button"
              onClick={() => onStatus(delivery, item)}
              className="rounded-full bg-[#d9ffb9] px-3 py-2 text-xs font-black capitalize text-[#153a20] hover:bg-[#ffe078]"
            >
              {item.replaceAll("_", " ")}
            </button>
          ))}
          <button
            ...
            className="grid size-9 place-items-center rounded-full bg-white text-zinc-500 shadow-sm hover:text-[#153a20]"
          >
            <Icon name="edit" className="size-4" />
          </button>
          <ConfirmAction
            label={delivery.orderNumber}
            disabled={deletingId === delivery.id}
            onConfirm={() => onRemove(delivery)}
          />
        </div>
      </div>
...
export default function DeliveryModule() {
  ...
  async function submit(event) {
    ...
    const res = await fetch(
      editingId ? `/api/deliveries/${editingId}` : "/api/deliveries",
      {
    ...
    setDeliveries((prev) =>
      editingId
        ? prev.map((item) => (item.id === editingId ? data.delivery : item))
        : [data.delivery, ...prev],
    );
  ...
  async function updateStatus(delivery, nextStatus) {
    ...
    setDeliveries((prev) =>
      prev.map((item) => (item.id === delivery.id ? data.delivery : item)),
    );
        ...
        <motion.section variants={fade} className={`${panel} p-5 sm:p-6`}>
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full bg-[#153a20] px-4 py-2.5 text-xs font-black text-white disabled:opacity-40"
            >
          ...
          <div className="mt-6">
            {filtered.length === 0 ? (
              <EmptyState
                ...
                description="Create a delivery or adjust filters to view active work."
              />
            ) : (
